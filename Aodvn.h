...
class  Aodvn : public RoutingProtocolBase, public NetfilterBase::HookBase, public UdpSocket::ICallback, public cListener
{
  protected:
    /*
  ...
  public:
    void handleMessageWhenUpn(cMessage *msg) ;
    void initializen(int stage) ;
    double delayhandle=0;
    virtual int numInitStages() const override { return NUM_INIT_STAGES; }

    /* Route Discovery */
    void startRouteDiscoveryn(const L3Address& target, unsigned int timeToLive = 0);
    void completeRouteDiscoveryn(const L3Address& target);
    bool hasOngoingRouteDiscoveryn(const L3Address& destAddr);
    void cancelRouteDiscoveryn(const L3Address& destAddr);

    /* Routing Table management */
    void updateRoutingTablen(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime);
    IRoute *createRouten(const L3Address& destAddr, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime);
    bool updateValidRouteLifeTimen(const L3Address& destAddr, simtime_t lifetime);
    void scheduleExpungeRoutesn();
    void expungeRoutesn();

    /* Control packet creators */
    const Ptr<RrepAck> createRREPACKn();
    const Ptr<Rrep> createHelloMessagen();
    const Ptr<Rreq> createRREQn(const L3Address& destAddr);
    const Ptr<Rrep> createRREPn(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& sourceAddr);
    const Ptr<Rrep> createGratuitousRREPn(const Ptr<Rreq>& rreq, IRoute *originatorRoute);
    const Ptr<Rerr> createRERRn(const std::vector<UnreachableNode>& unreachableNodes);

    /* Control Packet handlers */
    void handleRREPn(const Ptr<Rrep>& rrep, const L3Address& sourceAddr);
    void handleRREQn(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive, short trscore);
    void handleRERRn(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr);
    void handleHelloMessagen(const Ptr<Rrep>& helloMessage);
    void handleRREPACKn(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr);

    /* Control Packet sender methods */
    void sendRREQn(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive);
    void sendRREPACKn(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr);
    void sendRREPn(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive);
    void sendGRREPn(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive);

    /* Control Packet forwarders */
    void forwardRREPn(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive);
    void forwardRREQn(const Ptr<Rreq>& rreq, unsigned int timeToLive);

    /* Self message handlers */
    void handleRREPACKTimern();
    void handleBlackListTimern();
    void sendHelloMessagesIfNeededn();
    void handleWaitForRREPn(WaitForRrep *rrepTimer);

    /* General functions to handle route errors */
    void sendRERRWhenNoRouteToForwardn(const L3Address& unreachableAddr);
    void handleLinkBreakSendRERRn(const L3Address& unreachableAddr);
    virtual void receiveSignaln(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details) ;

    /* Netfilter hooks */
    Result ensureRouteForDatagramn(Packet *datagram);
    virtual Result datagramPreRoutingHook(Packet *datagram) override { Enter_Method("datagramPreRoutingHook"); return ensureRouteForDatagramn(datagram); }
    virtual Result datagramForwardHookn(Packet *datagram) ;
    virtual Result datagramPostRoutingHook(Packet *datagram) override { return ACCEPT; }
    virtual Result datagramLocalInHook(Packet *datagram) override { return ACCEPT; }
    virtual Result datagramLocalOutHook(Packet *datagram) override { Enter_Method("datagramLocalOutHook"); return ensureRouteForDatagramn(datagram); }
    void delayDatagramn(Packet *datagram);

    /* Helper functions */
    L3Address getSelfIPAddressn() const;
    void sendAODVPacketn(const Ptr<AodvControlPacket>& packet, const L3Address& destAddr, unsigned int timeToLive, double delay);
    void processPacketn(Packet *pk);
    void clearStaten();
    void checkIpVersionAndPacketTypeCompatibilityn(AodvControlPacketType packetType);

    /* UDP callback interface */
    virtual void socketDataArrivedn(UdpSocket *socket, Packet *packet) ;
    virtual void socketErrorArrivedn(UdpSocket *socket, Indication *indication) ;
    virtual void socketClosedn(UdpSocket *socket) ;
    virtual void socketsimAodv(short,short[1000],short);

    /* Lifecycle */
    virtual void handleStartOperationn(LifecycleOperation *operation) ;
    virtual void handleStopOperationn(LifecycleOperation *operation) ;
    virtual void handleCrashOperationn(LifecycleOperation *operation) ;
...
void Aodvn::initializen(int stage)
{
    if (stage == INITSTAGE_ROUTING_PROTOCOLS)
        addressType = getSelfIPAddressn().getAddressType(); // needed for handleStartOperationn()
...
void Aodvn::handleMessageWhenUpn(cMessage *msg)
{
    if (msg->isSelfMessage()) {
        if (auto waitForRrep = dynamic_cast<WaitForRrep *>(msg))
            handleWaitForRREPn(waitForRrep);
        else if (msg == helloMsgTimer)
            sendHelloMessagesIfNeededn();
    ...
    delayhandle+=(double)netDiameter;
}

void Aodvn::checkIpVersionAndPacketTypeCompatibilityn(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void Aodvn::processPacketn(Packet *packet)
{
    ...
    short nodeid=packet->getArrivalGateId();//notice Aodv
    short trscore=node_inf[nodeid];
    auto packetType = aodvPacket->getPacketType();
    switch (packetType) {
        case RREQ:
        case RREQ_IPv6:
            checkIpVersionAndPacketTypeCompatibilityn(packetType);
            handleRREQn(CHK(dynamicPtrCast<Rreq>(aodvPacket->dupShared())), sourceAddr, arrivalPacketTTL,trscore);
            delete packet;
            return;
        ...
        case RREP_IPv6:
            checkIpVersionAndPacketTypeCompatibilityn(packetType);
            handleRREPn(CHK(dynamicPtrCast<Rrep>(aodvPacket->dupShared())), sourceAddr);
            delete packet;
            return;
        ...
        case RERR_IPv6:
            checkIpVersionAndPacketTypeCompatibilityn(packetType);
            handleRERRn(CHK(dynamicPtrCast<const Rerr>(aodvPacket)), sourceAddr);
            delete packet;
            return;
        ...
        case RREPACK_IPv6:
            checkIpVersionAndPacketTypeCompatibilityn(packetType);
            handleRREPACKn(CHK(dynamicPtrCast<const RrepAck>(aodvPacket)), sourceAddr);
            delete packet;
            return;
...
INetfilter::IHook::Result Aodvn::ensureRouteForDatagramn(Packet *datagram)
{
    ...
    if (destAddr.isBroadcast() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        if (isActive && !route->getNextHopAsGeneric().isUnspecified()) {
            ...
            updateValidRouteLifeTimen(destAddr, simTime() + activeRouteTimeout);
            updateValidRouteLifeTimen(route->getNextHopAsGeneric(), simTime() + activeRouteTimeout);
            ...
            delayDatagramn(datagram);

            if (!hasOngoingRouteDiscoveryn(destAddr)) {
                ...
                if (isInactive)
                    startRouteDiscoveryn(destAddr, route->getMetric() + ttlIncrement);
                else
                    startRouteDiscoveryn(destAddr);
            }
            else
                EV_DETAIL << "Route discovery is in progress, originator " << getSelfIPAddressn() << " target " << destAddr << endl;
...
bool Aodvn::hasOngoingRouteDiscoveryn(const L3Address& target)
{
    return containsKey(waitForRREPTimers, target);
}

void Aodvn::startRouteDiscoveryn(const L3Address& target, unsigned timeToLive)
{
    EV_INFO << "Starting route discovery with originator " << getSelfIPAddressn() << " and destination " << target << endl;
    ASSERT(!hasOngoingRouteDiscoveryn(target));
    auto rreq = createRREQn(target);
    addressToRreqRetries[target] = 0;
    sendRREQn(rreq, addressType->getBroadcastAddress(), timeToLive);
}

L3Address Aodvn::getSelfIPAddressn() const
{
    return routingTable->getRouterIdAsGeneric();
}

void Aodvn::delayDatagramn(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void Aodvn::sendRREQn(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    if (rrepTimer != waitForRREPTimers.end()) {
        ...
        ASSERT(hasOngoingRouteDiscoveryn(rreq->getDestAddr()));
    ...
    sendAODVPacketn(rreq, destAddr, timeToLive, *jitterPar);
    rreqCount++;
}

void Aodvn::sendRREPn(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendAODVPacketn(rrep, nextHop, timeToLive, 0);
}

const Ptr<Rreq> Aodvn::createRREQn(const L3Address& destAddr)
{
    ...
    rreqPacket->setOriginatorAddr(getSelfIPAddressn());
    ...
    RreqIdentifier rreqIdentifier(getSelfIPAddressn(), rreqId);
    rreqsArrivalTime[rreqIdentifier] = simTime();
    return rreqPacket;
}

const Ptr<Rrep> Aodvn::createRREPn(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    if (rreq->getDestAddr() == getSelfIPAddressn()) { // node is itself the requested destination
...
const Ptr<Rrep> Aodvn::createGratuitousRREPn(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void Aodvn::handleRREPn(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    if (!previousHopRoute || previousHopRoute->getSource() != this) {
        ...
        previousHopRoute = createRouten(sourceAddr, sourceAddr, 1, false, rrep->getDestSeqNum(), true, simTime() + activeRouteTimeout);
    }
    else
        updateRoutingTablen(previousHopRoute, sourceAddr, 1, false, rrep->getDestSeqNum(), true, simTime() + activeRouteTimeout);
    ...
    if (destRoute && destRoute->getSource() == this) { // already exists
        ...
        if (!destRouteData->hasValidDestNum()) {
            updateRoutingTablen(destRoute, sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);
        ...
        destRoute = createRouten(rrep->getDestAddr(), sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);
        destRouteData = check_and_cast<AodvRouteData *>(destRoute->getProtocolData());
    }
    ...
    if (getSelfIPAddressn() != rrep->getOriginatorAddr()) {
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                if (rrep->getAckRequiredFlag()) {
                    auto rrepACK = createRREPACKn();
                    sendRREPACKn(rrepACK, sourceAddr);
                ...
                forwardRREPn(outgoingRREP, originatorRoute->getNextHopAsGeneric(), 100);
            }
        }
        else
        ...
        if (hasOngoingRouteDiscoveryn(rrep->getDestAddr())) {
            EV_INFO << "The Route Reply has arrived for our Route Request to node " << rrep->getDestAddr() << endl;
            updateRoutingTablen(destRoute, sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);
            completeRouteDiscoveryn(rrep->getDestAddr());
        }
    }
}

void Aodvn::updateRoutingTablen(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    ...
    scheduleExpungeRoutesn();
}

void Aodvn::sendAODVPacketn(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void Aodvn::socketDataArrivedn(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacketn(packet);
}

void Aodvn::socketErrorArrivedn(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void Aodvn::socketClosedn(UdpSocket *socket)
{
    if (operationalState == State::STOPPING_OPERATION)
        startActiveOperationExtraTimeOrFinish(par("stopOperationExtraTime"));
}

void Aodvn:: socketsimAodv(short numgate,short nodeinf[1000], short num_node )
{

    for(short i=0;i<num_node;i++)
    {
        node_inf[i]=nodeinf[i];
    }
    for (short i=0;i<numgate;i++)
    {
        cMessage *msg;
        handleMessageWhenUpn(msg);
        L3Address nodeadress=getSelfIPAddressn();
        auto lt = targetAddressToDelayedPackets.lower_bound(nodeadress);
        Packet *datagram = lt->second;
        const auto& networkHeader = getNetworkProtocolHeader(datagram);
        L3Address destAddr = networkHeader->getDestinationAddress();
        Ptr<Rreq> rreq= createRREQn( destAddr);
        sendRREQn(rreq, destAddr, 0);
    }

}

void Aodvn::handleRREQn(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive,short trscore)
{
    ...
    if (!previousHopRoute || previousHopRoute->getSource() != this) {
        // create without valid sequence number
        previousHopRoute = createRouten(sourceAddr, sourceAddr, 1, false, rreq->getOriginatorSeqNum(), true, simTime() + activeRouteTimeout);
    }
    else
        updateRoutingTablen(previousHopRoute, sourceAddr, 1, false, rreq->getOriginatorSeqNum(), true, simTime() + activeRouteTimeout);
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) { // create
        ...
        reverseRoute = createRouten(rreq->getOriginatorAddr(), sourceAddr, hopCount, true, rreqSeqNum, true, newLifeTime);
    }
    else {
        ...
        {
            updateRoutingTablen(reverseRoute, sourceAddr, hopCount, true, newSeqNum, true, newLifeTime);
        }
    }
    ...
    // check (i)
    if (rreq->getDestAddr() == getSelfIPAddressn()) {
        EV_INFO << "I am the destination node for which the route was requested" << endl;

        // create RREP
        auto rrep = createRREPn(rreq, destRoute, reverseRoute, sourceAddr);

        // send to the originator
        sendRREPn(rrep, rreq->getOriginatorAddr(), 255);
        ...
        destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        short min_trscor;//notice Aodv
        min_trscor=1000;
        for (short i=1;i<1000;i++)
        {
            if(node_inf[destRoute->getMetric()]<min_trscor)
            {
                min_trscor=node_inf[destRoute->getMetric()];
            }
        }
        ...
        if (!rreq->getDestOnlyFlag()) {
            ...
            if (rreq->getGratuitousRREPFlag()) {
                ...
                auto grrep = createGratuitousRREPn(rreq, originatorRoute);
                sendGRREPn(grrep, rreq->getDestAddr(), 100);
            }
    ...
    {
        short node_id=rreq.get()->nextId;
        node_inf[node_id]+=trscore;
    }
...
void Aodvn::receiveSignaln(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
        ...
        if (networkHeader != nullptr) {
            ...
            if (unreachableAddr.getAddressType() == addressType) {
                ...
                if (route && route->getSource() == this)
                    handleLinkBreakSendRERRn(route->getNextHopAsGeneric());
            }
        }
    }
}

void Aodvn::handleLinkBreakSendRERRn(const L3Address& unreachableAddr)
{
    ...
    auto rerr = createRERRn(unreachableNodes);
    rerrCount++;

    // broadcast
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendAODVPacketn(rerr, addressType->getBroadcastAddress(), 1, *jitterPar);
}

const Ptr<Rerr> Aodvn::createRERRn(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void Aodvn::handleRERRn(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
    ...
    if (unreachableNeighbors.size() > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        auto newRERR = createRERRn(unreachableNeighbors);
        sendAODVPacketn(newRERR, addressType->getBroadcastAddress(), 1, 0);
        rerrCount++;
    }
}

void Aodvn::handleStartOperationn(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void Aodvn::handleStopOperationn(LifecycleOperation *operation)
{
    socket.close();
    clearStaten();
}

void Aodvn::handleCrashOperationn(LifecycleOperation *operation)
{
    socket.destroy();
    clearStaten();
}

void Aodvn::clearStaten()
{
...
void Aodvn::handleWaitForRREPn(WaitForRrep *rrepTimer)
{
    ...
    sendRREQn(rreq, addressType->getBroadcastAddress(), 0);
}

void Aodvn::forwardRREPn(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendAODVPacketn(rrep, destAddr, 100, *jitterPar);
}

void Aodvn::forwardRREQn(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
    EV_INFO << "Forwarding the Route Request message with TTL= " << timeToLive << endl;
    sendAODVPacketn(rreq, addressType->getBroadcastAddress(), timeToLive, *jitterPar);
}

void Aodvn::completeRouteDiscoveryn(const L3Address& target)
{
    EV_DETAIL << "Completing route discovery, originator " << getSelfIPAddressn() << ", target " << target << endl;
    ASSERT(hasOngoingRouteDiscoveryn(target));
...
void Aodvn::sendGRREPn(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    sendAODVPacketn(grrep, nextHop, timeToLive, 0);
}

const Ptr<Rrep> Aodvn::createHelloMessagen()
{
    ...
    helloMessage->setDestAddr(getSelfIPAddressn());
...
void Aodvn::sendHelloMessagesIfNeededn()
{
    ...
    if (hasActiveRoute && (lastBroadcastTime == 0 || simTime() - lastBroadcastTime > helloInterval)) {
        ...
        auto helloMessage = createHelloMessagen();
        sendAODVPacketn(helloMessage, addressType->getBroadcastAddress(), 1, 0);
    }

    scheduleAfter(helloInterval - *periodicJitter, helloMsgTimer);
}

void Aodvn::handleHelloMessagen(const Ptr<Rrep>& helloMessage)
{
    ...
    if (!routeHelloOriginator || routeHelloOriginator->getSource() != this)
        createRouten(helloOriginatorAddr, helloOriginatorAddr, 1, true, latestDestSeqNum, true, newLifeTime);
    else {
        ...
        updateRoutingTablen(routeHelloOriginator, helloOriginatorAddr, 1, true, latestDestSeqNum, true, std::max(lifeTime, newLifeTime));
    }
        ...
        if (route->getSource() == this) {
            ...
            if (routeData->getLifeTime() <= simTime()) {
                if (routeData->isActive()) {
                    ...
                    if (hasOngoingRouteDiscoveryn(route->getDestinationAsGeneric())) {
...
void Aodvn::scheduleExpungeRoutesn()
{
...
INetfilter::IHook::Result Aodvn::datagramForwardHookn(Packet *datagram)
{
    ...
    if (destAddr.isBroadcast() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast()) {
        if (routingTable->isLocalAddress(destAddr) && ipSource && ipSource->getSource() == this)
            updateValidRouteLifeTimen(ipSource->getNextHopAsGeneric(), simTime() + activeRouteTimeout);
    ...
    updateValidRouteLifeTimen(sourceAddr, simTime() + activeRouteTimeout);
    updateValidRouteLifeTimen(destAddr, simTime() + activeRouteTimeout);

    if (routeDest && routeDest->getSource() == this)
        updateValidRouteLifeTimen(routeDest->getNextHopAsGeneric(), simTime() + activeRouteTimeout);
    ...
    if (routeDest && routeDestData && !routeDestData->isActive()) { // exists but is not active
        ...
        sendRERRWhenNoRouteToForwardn(destAddr);
    }
...
void Aodvn::sendRERRWhenNoRouteToForwardn(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    rerrCount++;
    EV_INFO << "Broadcasting Route Error message with TTL=1" << endl;
    sendAODVPacketn(rerr, addressType->getBroadcastAddress(), 1, *jitterPar); // TODO unicast if there exists a route to the source
}

void Aodvn::cancelRouteDiscoveryn(const L3Address& destAddr)
{
    ASSERT(hasOngoingRouteDiscoveryn(destAddr));
...
bool Aodvn::updateValidRouteLifeTimen(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> Aodvn::createRREPACKn()
{
...
void Aodvn::sendRREPACKn(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacketn(rrepACK, destAddr, 100, 0);
}

void Aodvn::handleRREPACKn(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void Aodvn::handleRREPACKTimern()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void Aodvn::handleBlackListTimern()
{
